<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="反馈类型" prop="fankuileixing">
          <el-select v-model="ruleForm.fankuileixing" placeholder="请选择反馈类型">
            <el-option
                v-for="(item,index) in fankuileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="反馈类型" prop="fankuileixing">
	      <el-input v-model="ruleForm.fankuileixing"
                placeholder="反馈类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="详细信息" prop="xiangxixinxi">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.xiangxixinxi" label="详细信息" prop="xiangxixinxi">
                    <span>{{ruleForm.xiangxixinxi}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='fankuileixing'){
            this.ruleForm.fankuileixing = obj[o];
	    this.ro.fankuileixing = true;
            continue;
          }
          if(o=='xiangxixinxi'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.fankuileixingOptions = "车票数量不足,个人信息有误,车票信息有误,其他".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fankui/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fankuiCrossAddOrUpdateFlag = false;
                  this.parent.search();
