<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="出发时间" prop="chufashijian">
          <el-input v-model="ruleForm.chufashijian" 
              placeholder="出发时间" clearable  :readonly="ro.chufashijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="出发时间" prop="chufashijian">
              <el-input v-model="ruleForm.chufashijian" 
...
export default {
  ...
  computed: {
    zongjia:{
      get: function () {
        return 1*this.ruleForm.jiage*this.ruleForm.renshu
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
