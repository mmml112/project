    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="退票原因" prop="tuipiaoyuanyin">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="退票原因"
                  v-model="ruleForm.tuipiaoyuanyin" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tuipiaoyuanyin" label="退票原因" prop="tuipiaoyuanyin">
                    <span>{{ruleForm.tuipiaoyuanyin}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shoujihao = true;
          ...
          if(o=='tuipiaoyuanyin'){
            this.ruleForm.tuipiaoyuanyin = obj[o];
	    this.ro.tuipiaoyuanyin = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tuipiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tuipiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
